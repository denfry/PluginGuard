...
## Phase 4 — Мультиформатность: моды, ресурспаки, датапаки (сделано)

> Расширение за пределы Bukkit-плагинов. Моды — те же `.jar` с классами, поэтому весь движок
> (байткод, обфускация, payload, taint, correlation) переиспользуется. Паки — zip без классов.

### 4.0 Усиление плагинов (закрытие дыр аудита)
- Платформенные sink'и в `BytecodeAnalyzer` (`Category.MINECRAFT`): `Bukkit/Server.dispatchCommand`,
  `setOp`, `getAccessToken` (кража session-токена клиентских модов).
- `MalwareSignatureAnalyzer` (`Category.MALWARE_SIGNATURE`): курируемые family-сигнатуры
  (fractureiser: пакеты `dev/neko/nekoclient`, C2 `85.217.144.130`/`files-8ie.pages.dev`/`skyrage.de`,
  пути `libWebGL64.jar`/`nekoservice`; session-RAT: `dev/majanito`/`whreceive.ru`). Честная рамка:
  матч = «индикатор известного семейства», C2 мертв.
- Расширенные IOC: `ops.json`/`usercache.json` + новое `IOC_BROWSER_CREDENTIAL` (logins.json/key4.db/discord).
- Combo `COMBO_OP_BACKDOOR`: op/console-control + сокрытие (обфускация/кодирование/пиратская переупаковка).

### 4.1 Каркас
- `LoaderDetector` (@Order 5) → `ArtifactType` (PLUGIN_*/MOD_*/RESOURCE_PACK/DATA_PACK/UNKNOWN) в контекст и `ScanResult`.
- `ScanController` принимает `.jar`/`.zip`/`.mcpack`/`.litemod`. `PluginYmlAnalyzer` type-aware (нет ложного `YML_MISSING`).

### 4.2 Моды (`ModAnalyzer`)
- Парсинг `fabric.mod.json`/`quilt.mod.json` (Jackson) и `mods.toml`/`neoforge.mods.toml` → `PluginInfo` + зависимости.
- Детект coremod/raw-трансформеров (`IFMLLoadingPlugin`/`IClassTransformer`/`ITransformationService`/`ITransformer`),
  `IMixinConfigPlugin`, access-widener.

### 4.3 Паки (`PackAnalyzer`) + zip-slip
- Ресурспак: `pack.mcmeta`, шейдеры, честный контекст (assets-only). Спрятанные payload ловит `EmbeddedPayloadAnalyzer`.
- Датапак: `.mcfunction` — auto-run теги `load`/`tick`, `op`/`deop`, само-рекурсия (lag), `tellraw` open_url (фишинг).
- Общий `STRUCTURE_PATH_TRAVERSAL` (zip-slip) для всех типов.

### 4.4 Веб
- `types.ts`: `ArtifactType` + категории `MINECRAFT`/`MALWARE_SIGNATURE`/`RESOURCE_PACK`/`DATA_PACK`.
- Dropzone принимает новые расширения; панель дескриптора динамическая; commands/permissions только для плагинов.

### Оговорки
- Динамический sandbox (Phase 3) — **только плагины**; моды/паки получают только статику.
- Ресурспаки низкорисковые (код не исполняют); датапаки — код-несущие (команды на permission level 2).

---

## Phase 5 — Многоосевой fitness-отчёт + Performance (lag-risk) анализ (сделано)

> Раньше PluginGuard возвращал один **security score** (0–100) и вердикт. Но безопасность — только
> одна ось. Плагин может быть безопасным, но жрать всё CPU в главном потоке. Фаза 5 вводит **многоосевую**
> архитектуру: каждый артефакт получает **независимые scores по осям** (Security, Performance, ...), и
> **общая рекомендация install** учитывает все оси. Если security возвращает CRITICAL, это — **абсолютный veto**.

### 5.1 Модель `Axis` и scoring
- Новый класс/enum `Axis` (SECURITY, PERFORMANCE, возможны ещё).
- Каждый `Axis` имеет собственный `Score` (0–100, как раньше).
- `ScanResult` расширяется: вместо одного `score` → `Map<Axis, Score>` или список `AxisScore`.
- `AxisScore`: axis, score, severity (CRITICAL/HIGH/MEDIUM/…), краткая рекомендация для оси.
- Общий verdict (INSTALL_RECOMMENDED / INSTALL_RISK_ACCEPTED / NOT_RECOMMENDED) вычисляется по всем осям;
  CRITICAL на любой оси = NOT_RECOMMENDED (security veto независимо от остального).

### 5.2 Performance анализатор
- Новый детектор `PerformanceAnalyzer` (запускается как отдельный проход после статических детекторов).
- Гибридный подход (`Hybrid-C`): построить граф вызовов (call-graph) до горячих путей (event handlers,
  repeating sync tasks), а затем ловить **blocking I/O** и **heavy work** на пути:
  - **Blocking I/O**: `JDBC` (java.sql), `HTTP` (java.net.http, OkHttp, HttpClient), file I/O (FileInputStream read,
    RandomAccessFile, Files.readAllBytes) на главном потоке (sync context).
  - **Heavy/sync work**: `Thread.sleep`, sync `chunk`-загрузки (world.getChunkAt), внешние API-вызовы без async-обёртки.
- Для **data packs**: `.mcfunction` scan on lag loops (рекурсивные теги, кольца `execute if`, высокая-частота `tick`-команды).
- Фокус: разделить `call-graph` на горячие и холодные пути; находка только на горячих = HIGH/MEDIUM;
  холодные пути = инфо-уровень или скрыто.
- `Category.PERFORMANCE` + новые `RuleId` для каждого паттерна.

### 5.3 Честная рамка для Performance
- **Статический прогноз, не измерение**: сказать clearly, что это — риск отставания, не TPS-метрика.
- **Слепые пятна**: не видит реальные load patterns, размеры данных, async/non-blocking library use, JIT.
- **Confidence levels**: прямые правила (e.g. `Thread.sleep` в handler) = HIGH confidence;
  deep call-graph (blocking I/O reachable через несколько методов) = LOW/MEDIUM confidence.

### 5.4 Поддержка платформ
- **Bukkit** (Paper/Spigot): event handlers как hot paths (PlayerJoinEvent, BlockBreakEvent, …).
- **Forge/Fabric/Proxy** (Velocity/Bungee): дополнительные hot paths (перехват пакетов, event подписки).
- **Data packs**: auto-run теги, sync `tick`-команды, рекурсивные вызовы (`function foo`).

### 5.5 Веб и интеграция
- `types.ts`: добавить `Axis`, `AxisScore`, обновить `ScanResult` структуру.
- Компонента отчёта: render таблица с осями и их scores; вывести общий verdict и security veto (если есть).
- Dropzone/кэш: сохранять/рендерить по всем осям.
- API: `POST /api/scan` теперь возвращает `ScanResult` с `axes` вместо одиночного `score`.

### 5.6 Тесты и синтетические случаи
- Synthetic JAR для Performance: plugin с `Thread.sleep` в `onEnable` → детектится как HIGH lag-risk.
- Plugin с blocking HTTP-вызовом в async-контексте → LOW/MEDIUM (холодный путь).
- Data pack с рекурсивной функцией в `tick` теге → lag loop flag.
- Бенигн-плагин (асинхронные I/O, non-blocking) получает низкий Performance score.
- Тест: plugin с CRITICAL security находит veto, несмотря на LOW performance.

### 5.7 Вердикт и рекомендация
- **INSTALL_RECOMMENDED**: все оси < HIGH, security < CRITICAL.
- **INSTALL_RISK_ACCEPTED**: есть HIGH, но нет CRITICAL; пользователь может это принять в ручную.
- **DO_NOT_INSTALL**: CRITICAL на security-оси — абсолютный veto. **AVOID**: HIGH на security-оси или CRITICAL на любой не-security оси.
- Напечатать ясно в отчёте: причина вердикта, какие оси это решили.

### Честная оговорка (повтор)
- Performance score — прогноз, не гарантия. Call-graph может быть неполным (динамический вызов,
  не видна через reflection). Малвари могут дормировать. Sandbox с инструментацией (Phase 3)
  поднимет confidence для плагинов (запустит и посмотрит реальное поведение).
